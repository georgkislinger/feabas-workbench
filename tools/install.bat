...
if errorlevel 1 goto :fail
rem desktop shortcut. PowerShell -Command parses single quotes inside double quotes itself, so
rem a quote anywhere in a path (%SC%, %PY%, %HERE%) would break the command silently; wrapping
rem every value in two quote styles keeps it one string for cmd and for PowerShell.
